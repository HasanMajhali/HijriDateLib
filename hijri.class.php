...
*
* @copyright	2015-2024 Saeed Hubaishan
...
*/
public function format($format = null, $force_hijri = false): string
{
if (!isset($format)) {
$format = $this->defaultformat;
}
$hsmonths = [1 => 'Muh', 'Saf', 'Rb1', 'Rb2', 'Jm1', 'Jm2', 'Raj', 'Sha', 'Ram', 'Shw', 'Qid', 'Hij'];
if ($this->langcode == 'ar') {
$gmonths = [1 => 'يناير', 'فبراير', 'مارس', 'أبريل', 'مايو', 'يونيو', 'يوليو', 'أغسطس', 'سبتمبر', 'أكتوبر', 'نوفمبر', 'ديسمبر'];
$smonths = [1 => 'كانون الثاني', 'شباط', 'آذار', 'نيسان', 'أيار', 'حزيران', 'تموز', 'آب', 'أيلول', 'تشرين الأول', 'تشرين الثاني', 'كانون الأول'];
$days = ['الأحد', 'الإثنين', 'الثلاثاء', 'الأربعاء', 'الخميس', 'الجمعة', 'السبت'];
}
[$gy, $gm, $gd, $w, $mn, $am] = explode('/', parent::format('Y/m/d/w/n/a'));
...
{
if (!empty(self::$s_umdata) && (($with_adj && ($this->adj_data == self::$s_adjdata)) || (!$with_adj && (self::$s_adjdata == [])))) {
...
break;
case 'bn':
$months = [1 => 'মহররম', 'সফর', 'রবিউল আউয়াল', 'রবিউস সানি', 'জমাদিউল আউয়াল', 'জমাদিউস সানি', 'রজব', 'শা‘বান', 'রমজান', 'শাওয়াল', 'জ্বিলকদ', 'জ্বিলহজ্জ'];
break;
case 'bs':
...
break;
case 'hi':
$months = [1 => 'मुहर्रम', 'सफ़र', 'रबीउल अव्वल', 'रबीउल आख़िर', 'जमादी-उल-अव्वल', 'जमादी-उल-आख़िर', 'रजब', 'शाबान', 'रमज़ान', 'शव्वाल', 'ज़िलक़ाद', 'ज़िलहिज्ज'];
break;
case 'id':
...
break;
case 'ru':
$months = [1 => 'Мухаррам', 'Сафар', 'Раби-уль-авваль', 'Раби-уль-ахир', 'Джумад-уль-авваль', 'Джумад-уль-ахир', 'Раджаб', 'Шаабан', 'Рамадан', 'Шавваль', 'Зуль-Каада', 'Зуль-Хиджжа'];
break;
case 'es':
...
[$hm, $hy] = $this->off2month($k);
$auto_adj[] = ['month' => $hm, 'year' => $hy, 'grdate' => $this->myjd2gre($v), 'jd' => $v];
}
$myret[] = ['grdate' => $this->myjd2gre($n), 'jd' => $n, 'currentset' => ($n == $this->umdata[$off]), 'alsoadjdata' => $auto_adj];
}
}
return $myret;
}
...
[$hm, $hy] = $this->off2month($k);
$myret[] = ['month' => $hm, 'year' => $hy, 'current' => $this->myjd2gre($v), 'default' => $this->myjd2gre($this->umdata_clear[$k])];
}
return $myret;
}
/**
...
[$gd, $gm, $gy] = preg_split('/[-\/.\\\ ]/', $new_month_start);
$value = gregoriantojd($gm, $gd, $gy) - static::mjd_factor;
}
$len = $value - $this->umdata[$off - 1];
if ($len > 28 && $len < 31) {
$new_adj_data = array_replace([$off => $value], $this->check_auto_adj($off, $value));
